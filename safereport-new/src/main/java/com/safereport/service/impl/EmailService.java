...
public class EmailService {
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            log.error("Email attempt {}/{} failed for: {} | Error: {}", attempt, maxRetries, to, e.getMessage());
        } catch (Exception e) {
            log.error("Email attempt {}/{} failed for: {} | Error: {}", attempt, maxRetries, to, e.getMessage());
        }

        // Wait before next retry: 2s, 4s, 6s
    ...
    log.error("All {} attempts failed. Could not send email to: {}", maxRetries, to);
}
    ...
    private String buildPasswordResetHtml(String userName, String resetToken) {
        // Format OTP with spaces between digits for readability: "1 2 3 4 5 6"
        String otpSpaced = resetToken.chars()
                        ...
                        <td align="center">
                            <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:12px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,0.08);">
                                ...
                                <tr>
                                    <td style="padding:35px 40px;">
                                        <p style="color:#555; font-size:15px; margin:0 0 10px;">Hello <strong>%s</strong>,</p>
                                        <h2 style="color:#6366f1; font-size:20px; margin:0 0 12px;">Password Reset OTP 🔐</h2>
                                        <p style="color:#555; font-size:15px; line-height:1.7; margin:0 0 20px;">
                                            We received a request to reset your SafeReport password.
                                            Use the 6-digit OTP below to reset it:
                                        </p>
                                        <!-- OTP Box -->
                                        <div style="background:linear-gradient(135deg,#eef2ff,#f5f3ff); border:2px solid #6366f1; padding:28px 20px; border-radius:12px; text-align:center; margin:20px 0;">
                                            <p style="color:#6366f1; font-size:13px; font-weight:600; margin:0 0 12px; text-transform:uppercase; letter-spacing:1px;">Your One-Time Password</p>
                                            <p style="color:#1e293b; font-size:42px; font-weight:800; letter-spacing:12px; margin:0; font-family:monospace;">%s</p>
                                        </div>
                                        <p style="color:#888; font-size:13px; margin:20px 0 0; line-height:1.6;">
