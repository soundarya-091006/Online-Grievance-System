...
public class EvidenceController {
    ...
    /**
     * List all evidence for a complaint — authenticated users only
     */
    @GetMapping("/complaint/{complaintId}")
    public ResponseEntity<ApiResponse<List<EvidenceResponse>>> listEvidence(
            @PathVariable("complaintId") Long complaintId) {
        return ResponseEntity.ok(ApiResponse.success(complaintService.getEvidence(complaintId)));
    }

    /**
     * Download a specific evidence file — authenticated users only
     */
